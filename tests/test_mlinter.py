...
from mlinter import trf060 as _trf060_mod
...
"""
    ...
    def test_iter_modeling_files_skips_generated_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue(_helpers_mod.is_generated_file(generated))
            self.assertFalse(_helpers_mod.is_generated_file(handwritten))
            self.assertFalse(_helpers_mod.is_generated_file(modular))
...
"""
    ...
    def test_is_generated_file_detects_banner(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(_helpers_mod.is_generated_file(path))
    ...
    def test_is_generated_file_false_for_handwritten_source(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(_helpers_mod.is_generated_file(path))

    def test_is_generated_file_false_for_missing_file(self):
        missing = Path("/nonexistent/modeling_foo.py")
        self.assertFalse(_helpers_mod.is_generated_file(missing))
    ...
    def test_is_generated_file_only_reads_head(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            path.write_text(
                "x = 0\n" * 400 + f"# {_helpers_mod.GENERATED_FILE_MARKER} foo/modular_foo.py\n",
                encoding="utf-8",
            )
            self.assertFalse(_helpers_mod.is_generated_file(path))
    ...
    def test_iter_modeling_files_skips_generated_in_explicit_paths(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            generated.write_text(
                f"# {_helpers_mod.GENERATED_FILE_MARKER} foo/modular_foo.py\nclass FooModel:\n    pass\n",
                encoding="utf-8",
            )
...
"""
    ...
    def test_src_scope_scans_every_python_file_under_an_explicit_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "src/transformers"
    ...
    def test_src_scope_skips_files_outside_a_transformers_package_tree(self):
        # A standalone model repo is not the transformers package: source rules encode package
        # conventions, so an explicit path outside `src/transformers` must not pull a file into scope.
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            standalone = root / "model.py"
            standalone.write_text("", encoding="utf-8")

            with patch.dict(mlinter.TRF_RULE_SPECS[mlinter.TRF012], {"file_scope": "src"}):
                found = dict(mlinter.iter_files({mlinter.TRF012}, search_paths=[root]))
                found_explicit = dict(mlinter.iter_files({mlinter.TRF012}, search_paths=[standalone]))

        self.assertEqual(found, {})
        self.assertEqual(found_explicit, {})
    ...
    def test_main_warns_when_a_search_path_holds_no_file_in_an_enabled_scope(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            stdout, stderr = StringIO(), StringIO()
            with (
        ...
        self.assertEqual(exit_code, 0)
        self.assertIn(
            "no model integration file or file under src/transformers found",
